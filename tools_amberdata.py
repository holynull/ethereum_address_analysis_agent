...
async def get_apr_basis_live_term_structure(
    asset, margin_type="coins", time_format="milliseconds"
):
    """
    ...
    try:
        ...
        if data and "payload" in data and len(data["payload"]["data"]) > 0:
            ...
            for i, exchange in enumerate(exchanges):
                ...
                # 设置可见性：只有前两个symbol默认可见
                visibility = True if i < 2 else "legendonly"

                # APR line chart
                fig.add_trace(
                    go.Scatter(
                        x=exchange_data["expirationDate"],
                        y=exchange_data["apr"],
                        mode="lines",
                        name=f"{exchange} APR",
                        line=dict(color=color),
                ...
                fig.add_trace(
                    go.Scatter(
                        x=exchange_data["expirationDate"],
                        y=exchange_data["basis"],
                        mode="lines",
                        name=f"{exchange} Basis",
    ...
    try:
        ...
        if (
            ...
            for i, (symbol, group) in enumerate(grouped):
                color = colors[i % len(colors)]
                visible = (
                    True if i == 0 else "legendonly"
                )  # Only the first symbol is visible by default

                # Add realizedFunding as bar chart
                fig.add_trace(
                    go.Bar(
                        x=group.index,
                        y=group["realizedFunding"],
                        name=f"{symbol} Realized Funding",
                        marker_color=color,
                        visible=visible,
                        opacity=0.6,
                    )
                ...
                fig.add_trace(
                    go.Scatter(
                        x=group.index,
                        y=group["accumulatedRealizedFunding"],
                        name=f"{symbol} Accumulated Realized Funding",
            ...
            fig.update_layout(
                yaxis2=dict(title="Accumulated Funding", overlaying="y", side="right")
            )
